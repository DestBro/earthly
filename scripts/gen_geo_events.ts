...
function generatePolygonWithHoles(
	centerLon: number,
	centerLat: number,
	outerRadius: number,
): [number, number][][] {
	...
	for (let i = 0; i < numHoles; i++) {
		...
		const holeOffsetLat = faker.number.float({
			min: -outerRadius * 0.5,
			max: outerRadius * 0.5,
		});
		const holeOffsetLon = faker.number.float({
			min: -outerRadius * 0.5,
			max: outerRadius * 0.5,
		});

		const hole = generateCircularPolygon(
			centerLon + holeOffsetLon,
			centerLat + holeOffsetLat,
			holeRadius,
			8,
		);
		holes.push(hole);
...
function getThemeColors(theme: string) {
	...
	const scheme =
		colorSchemes[theme as keyof typeof colorSchemes] || colorSchemes.pois;
	return {
...
function clampFeatureCollectionSize(
	...
	const truncated = truncate(featureCollection as any, {
		precision: 5,
		mutate: false,
	...
	for (const tolerance of SIMPLIFY_TOLERANCES) {
		const simplifiedFeatures = current.features.map((feature) => {
			try {
				return simplify(feature as any, {
					tolerance,
					highQuality: false,
					mutate: false,
				}) as Feature;
...
async function loadEastGermanState(
	stateName: string,
...
function generateGeoJSONFeatures(
	theme: string,
	...
	for (let i = 0; i < count; i++) {
		...
		switch (theme) {
			case "parks": {
				...
				const parkType = faker.helpers.arrayElement([
					"simple",
					"circular",
					"irregular",
					"with_holes",
			...
			case "trails": {
				const trailType = faker.helpers.arrayElement([
					"simple",
					"winding",
					"branched",
				]);
				...
				if (trailType === "winding") {
					...
					const branch1 = generateRiverPath(
						mainPath[branch1Start][0],
						mainPath[branch1Start][1],
						8,
					);
					const branch2 = generateRiverPath(
						mainPath[branch2Start][0],
						mainPath[branch2Start][1],
						6,
					);
			...
			case "waterways": {
				...
				const waterType = faker.helpers.arrayElement([
					"river",
					"lake",
					"coastline",
				]);
				...
				if (waterType === "river") {
					...
					feature = {
						...
						properties: {
							...
							flow_direction: faker.helpers.arrayElement([
								"north",
								"south",
								"east",
								"west",
					...
					feature = {
						...
						properties: {
							...
							shore_type: faker.helpers.arrayElement([
								"sandy",
								"rocky",
								"cliff",
								"mixed",
			...
			case "administrative": {
				...
				const adminType = faker.helpers.arrayElement([
					"district",
					"zone",
					"ward",
				]);
				const boundaryComplexity = faker.helpers.arrayElement([
					"simple",
					"complex",
					"multi",
				]);
...
export async function generateGeoEventData(
	...
	options?: GenerateGeoEventOptions & {
		useRealData?: boolean;
		stateName?: string;
	},
): Promise<{
	...
	if (options?.useRealData) {
		...
		featureCollection = (options?.featureCollection as FeatureCollection &
			Record<string, any>) ?? {
			type: "FeatureCollection",
			name: `${faker.location.city()} ${theme} dataset`,
	...
	const bbox =
		...
		calculateBbox(featureCollection.features as Feature[]);
	...
	const baseHashtags = options?.hashtags ?? [
		theme ?? "external",
	...
	if (options?.blobReferences) {
		options.blobReferences.forEach((blob) => {
			const scope =
				blob.scope === "collection"
					? "collection"
					: `feature:${blob.featureId}`;
