...
export class NDKGeoEvent extends NDKEvent {
  ...
  set datasetSize(size: number | undefined) {
    this.replaceOptionalTag(
      "size",
      typeof size === "number" ? String(size) : undefined,
    );
  }
  ...
  updateDerivedMetadata(geohashPrecision = 6): void {
    ...
    try {
      ...
      const coordinates = computedCentroid.geometry?.coordinates as
        | Position
        | undefined;
      if (coordinates) {
  ...
  static async deleteDataset(
    ...
    if (!datasetId)
      throw new Error("Dataset is missing a d tag and cannot be deleted.");
