...
export const useNip60Store = create<Nip60State & Nip60Actions>()((set, get) => ({
	...initialState,
	...
	sendEcash: async (amount: number, mint?: string): Promise<string | null> => {
		...
		if (!targetMint) {
			targetMint = Object.keys(mintBalances).find((m) => (mintBalances[m] ?? 0) >= amount)
		}
...
export const nip60Actions = {
	initialize: (pubkey: string, ndk: NDK) => useNip60Store.getState().initialize(pubkey, ndk),
	loadTransactions: () => useNip60Store.getState().loadTransactions(),
	subscribeToTransactions: () => useNip60Store.getState().subscribeToTransactions(),
	createWallet: (mints: string[]) => useNip60Store.getState().createWallet(mints),
	reset: () => useNip60Store.getState().reset(),
	getWallet: () => useNip60Store.getState().getWallet(),
	refresh: (options?: { consolidate?: boolean }) => useNip60Store.getState().refresh(options),
	...
	setDefaultMint: (mintUrl: string | null) => useNip60Store.getState().setDefaultMint(mintUrl),
	...
	withdrawLightning: (invoice: string) => useNip60Store.getState().withdrawLightning(invoice),
	sendEcash: (amount: number, mint?: string) => useNip60Store.getState().sendEcash(amount, mint),
	...
	reclaimToken: (tokenId: string) => useNip60Store.getState().reclaimToken(tokenId),
	removePendingToken: (tokenId: string) => useNip60Store.getState().removePendingToken(tokenId),
	getActivePendingTokens: () => useNip60Store.getState().getActivePendingTokens(),
}
