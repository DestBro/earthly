...
function buildRelayUrls({
  ...
  const isLocalOrigin = locationInfo
    ? isLoopbackHostname(locationInfo.hostname)
    : false;
 ...
 */
const relayUrl = safeEnv(
  () => process.env.RELAY_URL as string,
  DEV_DEFAULTS.RELAY_URL,
);
const blossomServer = safeEnv(
  () => process.env.BLOSSOM_SERVER as string,
  DEV_DEFAULTS.BLOSSOM_SERVER,
);
const isProduction = safeEnv(
  () => process.env.NODE_ENV === "production",
  false,
);
const isDevelopment = safeEnv(
  () => process.env.NODE_ENV !== "production",
  true,
);
...
export const config = {
  ...
  serverPubkey: safeEnv(
    () => process.env.SERVER_PUBKEY as string,
    DEV_DEFAULTS.SERVER_PUBKEY,
  ),
