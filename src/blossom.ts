...
const PORT = process.env.BLOSSOM_PORT
  ? parseInt(process.env.BLOSSOM_PORT, 10)
  : 3001;
...
async function handleUpload(req: Request): Promise<Response> {
  ...
  console.log(
    `📤 Uploaded blob: ${sha256} (${(bodyBuffer.byteLength / 1024).toFixed(2)} KB)`,
  );
...
async function handleDelete(sha256: string): Promise<Response> {
  ...
  const filePath = join(
    MAP_CHUNKS_DIR,
    resolved.ext ? `${sha256}.${resolved.ext}` : sha256,
  );
