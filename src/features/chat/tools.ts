...
export const geoTools: Tool[] = [
	...
	},
	{
		...
		function: {
			...
			description:
				'Find OpenStreetMap features near a point. Can filter by tags like amenity=cafe, shop=supermarket. Use this to validate tags/results before importing.',
			parameters: {
				type: 'object',
				properties: {
					lat: {
	...
	},
	{
		...
		function: {
			name: 'query_osm_bbox',
			description:
				'Find OpenStreetMap features within a bounding box. Can filter by tags. Prefer this before import_osm_to_editor for administrative boundaries and ambiguous names.',
			parameters: {
				type: 'object',
				properties: {
					west: {
		...
		switch (toolCall.function.name) {
			...
			case 'import_osm_to_editor': {
				...
				const queryBbox = async (bbox: [number, number, number, number]) => {
					...
					return Array.isArray(response.result.features) ? response.result.features : []
				}
