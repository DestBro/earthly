	...
	return (
		<div className={`pointer-events-auto flex h-full flex-col ${className}`}>
			...
			<div className="flex-1 overflow-y-auto px-3 py-3 text-sm text-gray-700">
				{loading && <div className="text-xs text-gray-600">Fetching location info...</div>}
				...
				{inspectorActive && !result && !loading && !error && (
					<div className="text-xs text-gray-600">Click anywhere on the map to reverse geocode.</div>
				)}
			</div>
		</div>
	)
